...
void MainWindow::updatePageTheme() {
  ...
  // Inject matugen-generated theme CSS (if present) so WhatsApp Web adopts the
  // system Material You palette. Re-read on every load so a matugen reload is
  // picked up after the next page (re)load.
  QFile cssFile(QDir::homePath() + QStringLiteral("/.config/matugen/generated/whatsie.css"));
  if (cssFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    const QString cssB64 =
        QString::fromLatin1(QString::fromUtf8(cssFile.readAll()).toUtf8().toBase64());
    const QString cssJs = QString(R"js(
      (function() {
        try {
    ...
    m_webEngine->page()->runJavaScript(cssJs);
  }
}
